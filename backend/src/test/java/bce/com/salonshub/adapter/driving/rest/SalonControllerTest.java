/* path: src/test/java/bce/com/salonshub/adapter/driving/rest/SalonControllerTest.java */
...
class SalonControllerTest {
    ...
    void getAllSalons_shouldReturnOk() {
        ...
        when(retrievalPort.findAllSalons()).thenReturn(Flux.just(salon));
        webClient.get().uri("/salons").exchange()
            .expectStatus().isOk()
    ...
    void getSalonById_found_shouldReturnOk() {
        ...
        when(retrievalPort.findSalonById(new SalonId(id))).thenReturn(Mono.just(createSalon(id)));
        webClient.get().uri("/salons/{id}", id).exchange()
            .expectStatus().isOk()
    ...
    void getSalonById_notFound_shouldReturnNotFound() {
        UUID id = UUID.randomUUID();
        when(retrievalPort.findSalonById(new SalonId(id))).thenReturn(Mono.empty());
        webClient.get().uri("/salons/{id}", id).exchange().expectStatus().isNotFound();
    }
    ...
    void updateSalon_shouldReturnOk() {
        ...
        when(ingestionPort.updateSalon(any(), any())).thenReturn(Mono.just(createSalon(id)));
    ...
    void ingestSalon_shouldAccept() {
        ...
        when(ingestionPort.ingestSalon(any())).thenReturn(Mono.empty());
    ...
    void ingestBatch_shouldAccept() {
        ...
        when(ingestionPort.ingestSalons(any())).thenReturn(Mono.empty());
    ...
    void deleteById_shouldReturnNoContent() {
        UUID id = UUID.randomUUID();
        when(ingestionPort.deleteSalon(new SalonId(id))).thenReturn(Mono.empty());
        webClient.delete().uri("/salons/{id}", id).exchange().expectStatus().isNoContent();
    }

    @Test
    void deleteByFields_shouldReturnNoContent() {
        when(ingestionPort.deleteSalonsByFields(Map.of("district", "Center"))).thenReturn(Mono.empty());
        webClient.delete().uri("/salons?district=Center").exchange().expectStatus().isNoContent();
    }

    @Test
    void dynamicFind_shouldReturnFiltered() {
        when(retrievalPort.findSalonsByFields(Map.of("rating", "4.5"))).thenReturn(Flux.just(createSalon(UUID.randomUUID())));
        webClient.get().uri("/salons/filter?rating=4.5").exchange()
            .expectStatus().isOk()
            .expectBody().jsonPath("$[0].name").isEqualTo("Test Salon");
    }
